...
//use std::os::fd::AsFd; // ensures the trait is in scope
...
// structs from UdpSocket.  So we create a socket2
...
impl<T: Sample + std::fmt::Debug> UdpSourceBuilder<T> {
    ...
    pub fn build(self) -> anyhow::Result<(UdpSource<T>, ReadStream<T>)> {
        ...
        // use max socket memory
        let sockref = SockRef::from(&socket);
        sockref.set_recv_buffer_size(25 * 1024 * 1024)?;  // 25 MB
        println!("Warning: Using fixed socket memory of 25 MB.");
        ...
        if !self.config.multicast_addr.is_empty() {
            ...
            let bind_addr_merged_str =
                format!("{}:{}", self.config.bind_addr, self.config.bind_port);

            // Parse the merged string into a SocketAddr
            let bind_addr_merged: SocketAddr = bind_addr_merged_str.parse().with_context(|| {
                format!(
                    "Failed to merge bind_addr '{}' with port {}",
        ...
        //info!(
        //    "[UdpSourceBuilder ] 🟢 tx stream address = {:p}",
        //    &tx as *const _
        //);
        //info!(
        //    "[UdpSourceBuilder ] 🟢 rx stream address = {:p}",
        //    &rx as *const _
        //);
    ...
    T: Sample<Type = T> + std::fmt::Debug,
{
    ...
    fn work(&mut self) -> Result<BlockRet> {
        ...
        match self.socket.recv(&mut self.buffer) {
            Ok(n) => {
                // println!(
                //     "[{} udp_source work 298] Received {} bytes, total = {}",
                //     now(),
                //     n,
                ...
                //info!(
                //    "[UdpSource Block.work] chunked.size: {}",
                //    chunked.clone().count()
                //);
                ...
                if let Ok(mut writer) = self.dst.write_buf() {
                    ...
                    for (i, chunk) in chunked.enumerate() {
                        ...
                        match T::parse(chunk) {
                            Ok(sample) => {
                                // below is for debugging the u8 x2 speed (or 1/2 samples) bug
                                // if sample_counter < 10 {
                                //     let s: ComplexU8 = unsafe { std::mem::transmute_copy(&sample) };
                                //     println!(
                                //         "[{:?}] Parsed sample {}: re={}, im={}",
                                //         now(),
                                //         sample_counter,
...
mod tests {
    ...
    fn test_1_udp_source_receives_data() -> Result<()> {
        ...
        Note: you may have to run socat before each run of Test 1, it seems that socat will catch the first run
        ...
        info!(
            "[{test_name}] sending payload: 8 repeating bytes of 0xAB (171)"
        ...
        thread::spawn(move || {
            ...
            #[cfg(feature = "nix")]
            setsockopt(&socket, ReusePort, &true)
                 .unwrap_or_else(|e| eprintln!("❌ Failed to set ReusePort: {e}"));
            ...
            socket
                .bind(&bind_socket_addr.into())
                .expect("❌ bind failed");
            info!(
                "[{test_name} (thread)] bind_socket_addr: {bind_socket_addr}"
        ...
        info!(
            "[{test_name}] Multicaster for repeated payload commenced."
            
        );

        // Step 2: Start multicast receiver ---
        info!("[{test_name}] Setting up UdpSourceBuilder...");
        //let builder = UdpSourceBuilder::<u8>::new(
        let (mut src, rx) =
            UdpSourceBuilder::<u8>::new(bind_addr, bind_port, multicast_addr, multicast_port)
        ...
        info!("[{test_name}] Before calling work() loop.");
        loop {
            thread::sleep(Duration::from_millis(10)); // prevent flooding
            src.work()
                .unwrap_or_else(|e| panic!("❌ Failed to call work(): {e}"));
            info!("[{test_name}] After calling src.work()");
            let (reader, _) = rx
                .read_buf()
                .unwrap_or_else(|e| panic!("❌ Failed to read_buf: {e}"));
            if reader.is_empty() {
                info!("[{test_name}] reader is empty.");
                continue;
            } else {
                info!("[{test_name}] reader is NOT empty.");
                info!(
                    "[{}] After creating reader, length = {}",
                    test_name,
                    reader.len()
                );
                if reader.len() >= 2 {
                    //if true { // force entry into this clause
                    info!("[{test_name}] In reader >=1 clause.");
                    let data: &[u8] = reader.slice();
                    info!("[{test_name}] data: {data:?}");
                    //let a = reader[0];
        ...
        info!("[{test_name}] After calling work() loop.");

        let (reader, _tags) = rx.read_buf().expect("instantiation of reader failed.");
    ...
    fn test_2_udp_source_receives_incrementing_bytes() -> anyhow::Result<()> {
        ...
        */
        ...
        if !payload_choice {
            ...
            info!("[{test_name}] sending payload: {phrase}");
        ...
        let (mut src, rx) =
            UdpSourceBuilder::<u8>::new("0.0.0.0", TEST_PORT, "239.0.0.1", TEST_PORT)
                .iface_addr("192.168.1.2")
                .reuse_addr(true)
                .reuse_port(true)
                .build()
                .unwrap_or_else(|e| panic!("❌ Failed to call build() on UdpSourceBuilder: {e}"));
        info!("[{test_name}] Created UdpSource.");
    ...
    */
    ...
    fn test_3_udp_source_receives_data_subscribe_first() -> Result<()> {
        ...
        println!("Payload         : {test_payload:02X?}");
        println!("Max Attempts    : {max_attempts}");
        println!("Delay Per Try   : {delay_ms}ms");
        println!("------------------------\n");
        ...
        info!(
            "[{}] 🧪 rx address from test = {:p}",
            test_name, &rx as *const _
        );

        info!("[{}] test's socket dst ptr  = {:p}", test_name, &src.dst);
        info!(
            "[{}] test's socket rx ptr   = {:p}  --- this should match fn work()'s dst ptr which was being written to",
            test_name, &rx
        );
        info!(
            "[{test_name}] UdpSourceBuilder initialized. Waiting 200ms for multicast join..."
        ...
        info!("[{test_name}] Sent {sent} byte(s) to {dest_addr}");
        ...
        for attempt in 0..max_attempts {
            ...
            if !data.is_empty() {
                info!("[{test_name}]   Received data (ASCII):",);
                ...
                assert_eq!(
                    data[0], test_payload[0],
                    "[Test Reader] First byte does not match test payload."
                );
                received = true;
                break;
            }
        ...
        assert!(
            received,
            "[Test Reader] Did not receive expected UDP packet after {max_attempts} attempts."
    ...
    */
    ...
    fn test_4_outside_udp_server() -> Result<()> {
        ...
        // Below or RTL-SDR
        let foreign_addr = "239.192.0.3";
        let foreign_port = 5003;
        ...
        // below is AirSpy2 test
        // old: let (mut src, rx) = UdpSourceBuilder::<ComplexI16>::new(
        // let (mut src, rx) = UdpSourceBuilder::<i16>::new( // new 7/14 8:38 PM
        // bind_addr, bind_port,
        ...
        info!(
            "[{test_name}] Waiting 1 second before starting receive loop..."
        ...
        for i in 0..20 {
            ...
            if received_count >= 5 {
                info!("Received {received_count} total bytes, exiting early");
                return Ok(());
            }
